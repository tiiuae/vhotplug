            ...
            try:
                ...
                if result.returncode != 0:
                    ...
                    if r[0] == "no_available_port":
                        ...
                        logger.error("Out: %s", result.stdout)
                        logger.error("Err: %s", result.stderr)
            except (OSError, FileNotFoundError, PermissionError, ValueError, KeyError, AttributeError) as e:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if r[0] == "ok":
                    ...
                    logger.error("Out: %s", result.stdout)
                    logger.error("Err: %s", result.stderr)
        except (FileNotFoundError, PermissionError, OSError, IndexError) as e:
