...
class DeviceState:
    ...
    def select_vm_for_device(self, dev_info, vm_name: str):
        with self.lock:
            self.selected_vms[dev_info.persistent_id()] = vm_name
            self._save()

    def get_selected_vm_for_device(self, dev_info):
        with self.lock:
            return self.selected_vms.get(dev_info.persistent_id())

    def clear_selected_vm_for_device(self, dev_info):
        with self.lock:
    ...
    def set_disconnected(self, dev_info):
        with self.lock:
            self.disconnected_devices.add(dev_info.persistent_id())
            self._save()

    def is_disconnected(self, dev_info) -> bool:
        with self.lock:
            return dev_info.persistent_id() in self.disconnected_devices

    def clear_disconnected(self, dev_info) -> bool:
        with self.lock:
            dev_id = dev_info.persistent_id()
